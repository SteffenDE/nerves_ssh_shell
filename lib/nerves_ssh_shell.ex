defmodule NervesSSHShell.Utils do
  @moduledoc false
...
defmodule NervesSSHShell.CLI do
  ...
  > If you want to run a Unix shell in a subsystem, have a look at `NervesSSHShell.ShellSubsystem`
  > instead.
  """
  ...
  def handle_msg(msg, state) do
    Logger.error("[NervesSSHShell.CLI] unhandled message: #{inspect(msg)}")
    {:ok, state}
  end
  ...
  def handle_ssh_msg(msg, state) do
    Logger.error("[NervesSSHShell.CLI] unhandled ssh message: #{inspect(msg)}")
    {:ok, state}
  end
  ...
  * `force_pty` - enables pseudoterminal allocation, defaults to `true`.
  ...
  config :nerves_ssh,
    subsystems: [
      ...
      {'shell', {NervesSSHShell.ShellSubsystem, [command: '/bin/cat', force_pty: false]}},
    ],
    # ...
  ```
  """
  ...
  def handle_ssh_msg(msg, state) do
    Logger.error("[NervesSSHShell.ShellSubsystem] unhandled ssh message: #{inspect(msg)}")
    {:ok, state}
  end
